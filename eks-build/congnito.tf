...
resource "aws_cognito_user_pool_client" "kubeflow-pool-client" {
  name = "kubeflow-${var.kubeflow-ns}-${var.cluster-name}"
  user_pool_id = aws_cognito_user_pool.pool.id
  generate_secret     = true
  explicit_auth_flows = ["ALLOW_CUSTOM_AUTH","ALLOW_USER_SRP_AUTH","ALLOW_REFRESH_TOKEN_AUTH"]
  prevent_user_existence_errors = "ENABLED"
  callback_urls = ["https://kubeflow.${var.cluster-name}.${var.eks-hosted-dnszone}/login/generic_oauth"]
  allowed_oauth_flows = ["code"]
  allowed_oauth_scopes = ["openid","email","profile","aws.cognito.signin.user.admin"]
  supported_identity_providers = ["COGNITO"]
  allowed_oauth_flows_user_pool_client = true
